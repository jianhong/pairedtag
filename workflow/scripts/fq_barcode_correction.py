...
def update_fastq(r1,out_r1, barcode_dic ,white_list_barcode,log_file): ## process one files
    ...
    while True:
        ...
        cur_barcode = (cur_r1_name[:18].upper())  ## makesure the barcode length
        ...
        if   cur_barcode in white_list_barcode : ## only the whitelist or corrected barcode are remained
                ...
                #cur_r1_name = (barcode_dic[cur_barcode]+ cur_r1_name[18:]) ## 28bp cell barcode
                #cur_r1_read = (barcode_dic[cur_barcode]+ cur_r1_read[18:]) ## 28bp cell barcode
